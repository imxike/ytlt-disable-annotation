...
// @description    Adds video titles, shows previews and embeds on click. Also supported: Vimeo, LiveLeak, Dailymotion, vidme, WorldStarHipHop, Vine, Coub
...
function onLoad() {
    if(location.hostname.indexOf('liveleak.com') > -1) {
        return location.pathname == '/ll_embed' && location.hash == '#play' ? location.href = 'javascript:void(jwplayer.api.selectPlayer().play());' : false;
...
var links = {
    ...
    parseInfo: function(a) {
        ...
        try { url = decodeURIComponent(url.replace(/^https?:\/\/(?:www\.facebook\.com\/.+|sys\.4chan\.org\/derefer\?url=)(http.+)/, '$1')); } catch(ex) {};
    ...
    decorate: function(a, li, vi) {
        if(['ll', 'wh'].indexOf(li.sid) > -1 && !vi.status && location.protocol == 'https:') vi.status = 5;
...
var sites = {
    'yt':{
        ...
        embed:function(vid, t) {
            var m = /((\d+)h)?((\d+)m)?(\d+)?/.exec(t); t = parseInt(m[2] || 0)*3600 + parseInt(m[4] || 0)*60 + parseInt(m[5] || 0);
            if(cfg.annotation){
                return 'https://www.youtube.com/embed/' + vid + '?iv_load_policy=3&autoplay=1&rel=1' + (t ? '&start=' + t : '');
            }else {
    ...
    'vn':{
        ...
        embed:function(vid) { return 'https://vine.co/v/' + vid + '/card?mute=0'; },
    ...
    'll':{
        ...
        embed:function(vid) { return 'http://www.liveleak.com/ll_embed?i=' + vid + '#play'; },
        url:function(vid) { return 'http://www.liveleak.com/view?i=' + vid; },
...
var cfg = {
    settings: {
        annotation :{ title:'Disable annotation', default:true },
